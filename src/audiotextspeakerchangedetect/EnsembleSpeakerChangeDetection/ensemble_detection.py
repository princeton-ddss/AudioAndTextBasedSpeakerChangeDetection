...
def ensemble_detection(
    ...
    ensemble_voting: list = ["majority", "unanimity"],
):
    """
    ...
    Returns:
        None
    """
    detection_csv = pd.read_csv(
        os.path.join(detection_file_input_path, detection_file_input_name)
    )

    # Find all speaker change columns of all models
    speaker_change_columns = [
        colname
        for colname in detection_csv.columns
        if colname.startswith("speaker_change")
    ]
    speaker_change_nonlp_columns = [
        colname
        for colname in speaker_change_columns
        if "nlp" not in colname and "ensemble" not in colname
    ]
    ...
    if "speaker_change_llama2" in speaker_change_columns:
        ...
        detection_csv["speaker_change_llama2_copy"] = detection_csv[
            "speaker_change_llama2"
        ].copy()
        detection_csv["speaker_change_llama2"] = (
            detection_csv["speaker_change_llama2"]
            .apply(map_notsure_to_none)
            .apply(map_string_to_bool)
        )
    ...
    if "singularity" in ensemble_voting:
        ...
        detection_csv["speaker_change_ensemble_singularity"] = detection_csv[
            speaker_change_nonlp_columns
    ...
    if "majority" in ensemble_voting:
        ...
        if "speaker_change_llama2" in speaker_change_nonlp_columns:
            detection_csv["speaker_change_llama2_nan"] = detection_csv[
                "speaker_change_llama2"
            ].apply(map_none_to_nan)
            speaker_change_nonlp_columns_nan = speaker_change_nonlp_columns.copy()
            speaker_change_nonlp_columns.remove("speaker_change_llama2")
            speaker_change_nonlp_columns.append("speaker_change_llama2_nan")
            detection_csv_modes = detection_csv[speaker_change_nonlp_columns_nan].mode(
                axis=1, dropna=True
            )
            detection_csv = detection_csv.drop(columns=["speaker_change_llama2_nan"])

        else:
            detection_csv_modes = detection_csv[speaker_change_nonlp_columns].mode(
                axis=1, dropna=True
            )
        ...
        if len(detection_csv_modes.columns) > 1:
            detection_csv_modes = detection_csv_modes.rename(
                columns={0: "first_mode", 1: "second_mode"}
            )
            detection_csv_modes = detection_csv_modes.apply(
                lambda x: aggregate_two_modes(x["first_mode"], x["second_mode"]),
                axis=1,
            )
    ...
    if "speaker_change_nlp" in speaker_change_columns:
        ensemble_columns = [
            colname
            for colname in detection_csv.columns
            if "speaker_change_ensemble" in colname
        ]
        for ensemble_column in ensemble_columns:
            detection_csv[ensemble_column] = detection_csv.apply(
                lambda df: use_value_major(
                    value_major=df["speaker_change_nlp"],
                    value_minor=df[ensemble_column],
                ),
                axis=1,
            )
            # Could not convert string/object type to bool type by using astype(bool): "False" would become True
            detection_csv[ensemble_column] = detection_csv[ensemble_column].apply(
                map_string_to_bool
            )

    # Recover speaker_change_llama2 column
    if "speaker_change_llama2" in speaker_change_columns:
        detection_csv["speaker_change_llama2"] = detection_csv[
            "speaker_change_llama2_copy"
        ]
    ...
    detection_csv.to_csv(
        os.path.join(ensemble_output_path, detection_file_input_name), index=False
    )
